...
infoEnc = {
    ...
    записывает ключевые точки в текстовый файл, который будет зашифрован AES алгоритмом. { фотография должна быть в jpg }""",
...
class Actions(QtCore.QObject):
    messageSignal = QtCore.pyqtSignal(str, str, str)

    def __init__(self):
        super().__init__()
        self.messageSignal.connect(self.ms_box)
    ...
    def create_key(self):
        try:
            ...
            mn.PassLenRSA.clear()
            self.messageSignal.emit(APP_NAME, "Done crate RSA key", "info")
        except:
            self.messageSignal.emit(APP_NAME, "The key length is not entered", "err")
    ...
    def qtOpen(self, choice=True, filter=None):
        if choice:
            a = QtWidgets.QFileDialog.getOpenFileName(filter=filter)[0]
        else:
            a = QtWidgets.QFileDialog.getExistingDirectory()
        return a

    def takeFile(self, choice=0, steg=False):
        ...
        try:
            if not steg:
                ...
                file = self.qtOpen(filter=files_types)
                directory = os.path.dirname(file)
                if choice == 2: # stegano Image line
    ...
    def take_public(self):
        global publicKey, publicAddress
        try:
            publicAddress = self.qtOpen()
            with open(publicAddress, "rb") as publ:
                publicKey = publ.read()
...
class Encryption_func(QtCore.QThread):
    def __init__(self, parent=None):
        QtCore.QThread.__init__(self, parent=parent)
        ...
        try:
            if len(list(mn.PassworLine.text())) >= 16 and len(list(mn.PassworLine.text())) <= 24:
                ...
                if not dirCrypt:
                    ...
                    act.messageSignal.emit(APP_NAME, "Done!", "info")
            else:
                act.messageSignal.emit(APP_NAME, "password length is too small", "err")
        except Exception as err:
            act.messageSignal.emit(APP_NAME, str(err), "err")
    ...
    def decrypt_3DES(self, dirCrypt=False):
        try:
            ...
            if not dirCrypt:
                act.messageSignal.emit(APP_NAME, str(exp), "err")
    ...
    def encrypt_image(self):
        ...
        try:
            ...
            act.messageSignal.emit("DarkCryptor", "Done", "info")
        except TypeError:
            act.messageSignal.emit(APP_NAME, "Проверь формат файла", "err")
        except Exception as exp:
            act.messageSignal.emit(APP_NAME, str(exp), "err")
    ...
    def decrypt_image(self):
        try:
            ...
            act.messageSignal.emit("DarkCryptor", str(exp), "err")
    ...
    def encrypt(self, dirCrypt=False):
        try:
            ...
            if not dirCrypt:
                ...
                act.messageSignal.emit(APP_NAME, str(exp), "err", info)
    ...
    def encrypt_dir(self):
        ...
        try:
            ...
            if choiceSettings == 3:
                mn.plainTextEdit_2.setPlainText(f"Directory: {directory} has encrypted with password file {publicAddress}")
            elif choiceSettings == 10 or 11 or 12:
                mn.InfoPlain.setPlainText(f"Directory: {directory} has encrypted with password {mn.PassworLine.text()}")
            mn.FileLineRSA.clear()
            mn.CustomNameLineRSA.clear()
            mn.FileLine.clear()
            mn.CustomNameLine.clear()
...
if __name__ == '__main__':
    ...
    mn.CryptButtonRSA.clicked.connect(cr.run)
    ...
    mn.CryptButton.clicked.connect(cr.run)
    mn.TakeFileR.clicked.connect(lambda: act.takeFile(3, True))
    mn.TakeFileNR.clicked.connect(lambda: act.takeFile(4, True))
    mn.TakeImageBtn.clicked.connect(lambda: act.takeFile(2, True))
    mn.CryptButtonSteg.clicked.connect(cr.run)
